...
Displays only files larger than 10MB and sorts them from largest to smallest. This creates a "disk usage audit" view of your directory. File sizes are displayed in human-readable format (KB, MB, GB) by default. To show exact byte counts instead, use Edit-PowerTreeConfiguration and set HumanReadableSizes to false.
...
Displays file and folder sizes in readable units (KB, MB, GB) instead of raw byte counts. To show exact byte counts instead, use Edit-PowerTreeConfiguration and set HumanReadableSizes to false.
...
PowerTree provides extensive customization through a JSON configuration file. Use `Edit-PowerTreeConfiguration` to modify default settings.
